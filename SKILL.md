...
7. For marked public stash-tab price checks:
   ...
   - After a stash evaluation, optionally show prices directly over the stash grid with the local overlay helper. It reads saved report JSON and draws click-through labels at the fetched stash coordinates:
     ...
     - `Постоянный оверлей`: run `poe_stash_overlay_manager.py enable-autostart --start-now`; explain that this creates only this agent's Run entry, waits for the game window, follows it, and each overlay process closes with the game.
     ...
     - `Отключить полностью` means remove autostart and close the running overlay with `poe_stash_overlay_manager.py disable-autostart --stop-running`.
